...
function git::test::coverage::get_files_report {
  ...
  coverage_report_output="$(mktemp /tmp/git-friends-coverage-report.XXXXXXXXXX)"
...
function git::test::coverage::get_new_files_report {
  local \
    base_sha="${1:?}" \
    head_sha="${2:?}" \
  ...
  git::test::coverage::get_files_report "${coverage_report_input}" "${threshold}" "${new_files[@]}" \
    || echo 'No new covered files...'
}

function git::test::coverage::get_modified_files_report {
  local \
    base_sha="${1:?}" \
    head_sha="${2:?}" \
    coverage_report_input="${3:?}" \
  ...
  git::test::coverage::get_files_report "${coverage_report_input}" "${threshold}" "${modified_files[@]}" \
    || echo 'No modified covered files...'
}

function git::test::coverage::format_pull_request_comment {
  local \
    base_sha="${1:?}" \
    head_sha="${2:?}" \
    current_status="${3:?}" \
...
function git::test::coverage::generate_pull_request_comment {
  ...
  git::test::coverage::format_pull_request_comment \
    "${base_sha}" \
    "${head_sha}" \
    "${current_status}" \
    "${overall_report}" \
