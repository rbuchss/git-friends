...
find_and_format() {
  ...
  all_functions_output="$(mktemp /tmp/git-friends-all-function-exports.XXXXXXXXXX)"
  missing_functions_output="$(mktemp /tmp/git-friends-missing-function-exports.XXXXXXXXXX)"

  grep -E "${find_pattern}" "${file}" \
    | grep -v '__' \
    | sed -E "${replace_pattern}" \
    | grep -v 'export -f _[^_]' `# exclude bash-completion convention functions (_git_*)` \
      >>"${all_functions_output}"
